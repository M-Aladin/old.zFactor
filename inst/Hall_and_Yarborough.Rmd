...
```{r}
z.hallyarboroughL(pres.pr = 0.5, temp.pr = 1.3)$z
```
...
tbl <- sapply(ppr, function(x) 
    sapply(tpr, function(y) z.hallyarboroughL(pres.pr = x, temp.pr = 1.3)$z))
tbl
```

```{r}
library(ggplot2)
plot(x = ppr, y = tbl[1,], type = "l")
plot(x = ppr, y = tbl[2,], type = "l")
plot(x = ppr, y = tbl[3,], type = "l")
plot(x = ppr, y = tbl[4,], type = "l")
